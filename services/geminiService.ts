...
import { TTSVoice, PatientInfo } from "../types";

const getSystemInstruction = (patientInfo?: PatientInfo | null) => {
  const patientContext = patientInfo 
    ? `当前患者档案：姓名 ${patientInfo.name}，年龄 ${patientInfo.age}岁，性别 ${patientInfo.gender}，联系电话 ${patientInfo.phone}。请在对话中适时称呼患者，并结合其年龄和性别特征给出专业的医疗意见。`
    : "";
...
5. 格式：使用 Markdown 进行列表和强调。

特殊功能：
- 医疗数据分析：您可能会收到结构化数据输入（验血、验尿、脉搏、粪便检查）。请根据标准医疗范围分析这些数值。
- 医疗处方：如果被要求“打印处方”或“生成报告”，请使用 Markdown 标题（# 医疗处方）输出正式的文件结构。包含以下部分：患者详情（包含姓名、年龄、电话）、诊断结果、处方药物（剂量、频率、持续时间）以及建议/注意事项。
`;
};
...
export const sendMessageToGemini = async (
  ...
  try {
    ...
    const chat = ai.chats.create({
      ...
      history: chatHistory,
      config: {
        systemInstruction: getSystemInstruction(patientInfo),
      }
    });
